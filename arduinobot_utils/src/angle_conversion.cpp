...
{
public:
    AngleConverter() : Node("angles_conversion_service")
    {
        euler_to_quaternion_ = create_service<arduinobot_msgs::srv::EulerToQuaternion>("euler_to_quaternion", std::bind(&AngleConverter::eulerToQuaternionCallback, this, _1, _2));

        quaternion_to_euler_ = create_service<arduinobot_msgs::srv::QuaternionToEuler>("quaternion_to_euler", std::bind(&AngleConverter::quaternionToEulerCallback, this, _1, _2));

        RCLCPP_INFO(this->get_logger(), "Angle Conversion Service is ready...");
    }
...
private:
    ...
    {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Request to convert euler angles roll:" << req->roll << " pitch:" << req->pitch << " yaw:" << req->yaw << " into quartenion");
        tf2::Quaternion q;
        ...
        res->z = q.getZ();
        res->w = q.getW();
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Corresponding quartenion x:" << res->x << " y:" << res->y << " z:" << res->z << " w:" << res->w);
    }
                                   ...
                                   const std::shared_ptr<arduinobot_msgs::srv::QuaternionToEuler::Response> res)
    {
        ...
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Corresponding Euler angle roll:" << res->roll << " pitch:" << res->pitch << " yaw:" << res->yaw);
    }
